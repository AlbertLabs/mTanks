	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (counter == 5)
				counter = 0;
			tanks = 0;
			for (WorldObject a : (ArrayList<WorldObject>) world.getList()
					.clone()) { // only Tanks run
				if (a instanceof Tank) {
			 ...
			 * for(WorldObject a: world.getList()){ if(a.alive()){
			 * tempList.add(a); } } for(WorldObject a: tempList){
			 * world.remove(a); }
			 */
			...
			for (WorldObject d : (ArrayList<WorldObject>) world.getList()
					.clone()) {
			...
			if (counter == 0) {
				for (int i = 0; i < printData.size(); i++) {
					 ...
					 * this.angle=angle; this.health=he; this.maxHealth=mhe;
					 * this.image = image;
					 */
					world.repaint();
					ref.child("objects").child(Integer.toString(i)).child("y")
							.setValue(printData.get(i).y);
					ref.child("objects").child(Integer.toString(i)).child("x")
							.setValue(printData.get(i).x);
					ref.child("objects").child(Integer.toString(i))
							.child("width").setValue(printData.get(i).width);
					ref.child("objects").child(Integer.toString(i))
							.child("height").setValue(printData.get(i).height);
					ref.child("objects").child(Integer.toString(i))
							.child("angle").setValue(printData.get(i).angle);
					ref.child("objects").child(Integer.toString(i))
							.child("health").setValue(printData.get(i).health);
					ref.child("objects").child(Integer.toString(i))
							.child("maxHealth")
							.setValue(printData.get(i).maxHealth);
					ref.child("objects").child(Integer.toString(i))
							.child("image").setValue(printData.get(i).image);
